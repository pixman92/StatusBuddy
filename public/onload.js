...
//========================================
async function populateSearchHTML(){
    await pullStatusAny(document.getElementById('searchInput').value);
    searchArr=[];
    ...
    searchArr.push('<div class=\"w3-card marginAroundMe\">');
    searchArr.push(secsToDate(wholeDoc[0].date.seconds).toString());
    searchArr.push('</div>');

    // searchArr.join("");
    document.getElementById('statusFromSearch').innerHTML = searchArr.join("");
    document.getElementById('searchInput').value = wholeDoc[0].email;
}
