...
async function pullFromSavedEmails(myEmail){
    ...
    async function four(three3){
        ...
        var arrOfClickableEmails = []
        
        getAllArr.forEach(async(elem)=>{
            document.getElementById(elem.savedEmail).addEventListener('click', ()=>{});
            await pullStatus(elem.savedEmail);
        });

        // for(var i=0; i<getAllArr.length; i++){
        //     console.log('i', i);
        //     arrOfClickableEmails.push(getAllArr[i].savedEmail);
        // }
        // console.log('arrOfClickableEmails', arrOfClickableEmails);

        // arrOfClickableEmails.forEach((elem)=>{
        //     document.getElementById(elem).addEventListener('click', async()=>{
        //             await pullStatus(elem);
        //     });
